...
sub search_rs {
  ...
  if ($call_attrs and keys %$call_attrs) {
    ...
    if (my $c = delete $new_attrs->{cols}) {
      carp_unique( "Resultset specifies the 'cols' attribute - this has been deprecated. Please use 'columns' instead'" );
...
sub _normalize_selection {
  ...
  if ( exists $attrs->{include_columns} ) {
    carp_unique( "Resultset specifies the 'include_columns' attribute - this has been deprecated. Please use '+columns' instead'" );
